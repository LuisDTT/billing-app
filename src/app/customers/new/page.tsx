...
const Page = ({ params }: { params: { id: string } }) => {
      ...
      if (params.id) {
        ...
        if (customerDataToUpdate) {
          ...
          setFormattedFeeValue(
            formatNumber(customerDataToUpdate.feeValue).formattedValue
          );
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (Object.keys(errors).length === 0) {
      ...
      if (isValidPlate) {
        ...
        if (params.id) {
          ...
          const response = await createNewDoc("customers", dataToSend);
          if (response?.success) {
            setFormData({
              name: "",
              phoneNumber: "",
    ...
    setLoading(false);
    navigate("/customers"); // Mejorar redireccionamiento
  };
  ...
  return (
    <>
      ...
      <div className={loadingCustomerData ? "hidden" : ""}>
        <CustomerForm
          formTitle={"Registrar nuevo cliente"}
          ...
          loading={loading}
          isUpdating={params.id ? true : false}
